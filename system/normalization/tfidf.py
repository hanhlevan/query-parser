...
from system.algorithm.matching import Consine
...
class tfIdf:
    def __init__(self, indexer, min_tfidf=0.05, max_tfidf=100):
        ...
        self.stopwords = dict()
        self.doc_size = 0
    def set_fields(self, index_fields, show_fields):
        self.indexer.set_fields(index_fields, show_fields)    
    ...
    def get_similarity(self, word_counts, list_indices):
        ...
        for _id in list_indices:
            ...
            for field in list_indices[_id]:
                score = Consine().exec(vector_sen[field], vector_doc[field])
                sim[field][_id] = score
                scores[_id] += score
        return scores
